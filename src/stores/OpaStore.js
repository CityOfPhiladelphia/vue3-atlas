...
export const useOpaStore = defineStore('OpaStore', {
  ...
  actions: {
    ...
    async fillOpaData() {
      ...
      if (API_SOURCES.opaData === 'databridge') {
        return this._fillOpaDataDatabridge();
      }
      return this._fillOpaDataCarto();
    },
    async _fillOpaDataDatabridge() {
      try {
        const GeocodeStore = useGeocodeStore();
        const OpaNum = GeocodeStore.aisData.features[0].properties.opa_account_num;
        const data = await fetchTableWithFallback('opaData', { table: 'opa_properties_public', where: `parcel_number = '${OpaNum}'` });
        if (data) {
          this.opaData = data;
          return;
        }
