  ...
  actions: {
    async checkAisData(parameter) {
      try {
        ...
        const response = await fetch(`https://0spy4bb9w1.execute-api.us-east-1.amazonaws.com/queryAis/search/${encodeURIComponent(parameter)}?include_units=false&client_id=${import.meta.env.VITE_DEBUG ? import.meta.env.VITE_AIS_CLIENTID_ATLAS : ''}`)
        if (response.ok) {
    ...
    async fillAisData(address) {
      try {
        ...
        const response = await fetch(`https://0spy4bb9w1.execute-api.us-east-1.amazonaws.com/queryAis/search/${encodeURIComponent(address)}?include_units=false&client_id=${import.meta.env.VITE_DEBUG ? import.meta.env.VITE_AIS_CLIENTID_ATLAS : ''}`)
        if (response.ok) {
