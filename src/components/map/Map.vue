...
onMounted(async () => {
  ...
  map.on('mouseenter', 'nearbyActivity', (e) => {
  ...
  map.on('mouseleave', 'nearbyActivity', () => {
...
const toggleImagery = () => {
  ...
  if (!MapStore.imageryOn) {
    ...
    const currentYear = imagerySelected.value ? imagerySelected.value : Object.keys($config.mapLayers).at(-10); // -10 index should always point to most recent year provided the number of layers with alphabet character names does not change
...
watch(
  () => clickedRow.value,
  newClickedRow => {
    // if (import.meta.env.VITE_DEBUG) console.log('Map.vue clickedRow watch, newClickedRow:', newClickedRow, 'newClickedRow.type:', newClickedRow.type);
    ...
    // if (import.meta.env.VITE_DEBUG) console.log('nearby click, newClickedRow:', newClickedRow, 'idField:', idField, 'row:', row);
...
<template>
  ...
  <KeepAlive>
    <EagleviewPanel v-if="MapStore.eagleviewOn" />
  </KeepAlive>
