...
onMounted(() => {
  const map = MapStore.map;
  if (!NearbyActivityStore.loadingData && nearbyConstructionPermitsGeojson.value.length > 0) { map.getSource('nearbyActivity').setData(featureCollection(nearbyConstructionPermitsGeojson.value)) }
});
onBeforeUnmount(() => {
  const map = MapStore.map;
