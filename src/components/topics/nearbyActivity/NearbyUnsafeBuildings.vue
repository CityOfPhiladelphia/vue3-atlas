...
onMounted(() => {
  const map = MapStore.map;
  if (!NearbyActivityStore.loadingData && nearbyUnsafeBuildingsGeojson.value.length > 0) { map.getSource('nearbyActivity').setData(featureCollection(nearbyUnsafeBuildingsGeojson.value)) }
});
onBeforeUnmount(() => {
  const map = MapStore.map;
