...
export async function getAgoToken() {
  try {
    const response = await fetch('https://jyw9g46827.execute-api.us-east-1.amazonaws.com/getAgoTok')
...
export async function getEagleviewToken() {
  try {
    const response = await fetch('https://jyw9g46827.execute-api.us-east-1.amazonaws.com/getEagleTok');
    const data = await response.json();
...
export async function getCyclomediaRecordings(url, srid, swLng, swLat, neLng, neLat) {
  ...
  try {
    const response = await fetch(`https://jyw9g46827.execute-api.us-east-1.amazonaws.com/getCycloRecs?${searchParams.toString()}`)
    const data = await response.json()
